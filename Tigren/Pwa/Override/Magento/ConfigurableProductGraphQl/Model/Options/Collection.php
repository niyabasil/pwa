...
namespace Tigren\Pwa\Override\Magento\ConfigurableProductGraphQl\Model\Options;
 ...
 */
class Collection extends \Magento\ConfigurableProductGraphQl\Model\Options\Collection
{
    /**
     * Option type name
     */
    ...
    public function __construct(
        ...
        parent::__construct($attributeCollectionFactory, $productFactory, $metadataPool, $uidEncoder);
    ...
    private function fetch(): array
    {
        ...
        foreach ($attributeCollection->getItems() as $attribute) {
            ...
            $this->attributeMap[$productId][$attribute->getId()]['used_in_product_listing'] =
                (int)$attribute->getProductAttribute()->getData('used_in_product_listing');

            $attributeAdditionalData = $attribute->getProductAttribute()->getData('additional_data');
            $frontendInput = !empty($attributeAdditionalData) ? \json_decode($attributeAdditionalData, true) : [];
            if (count($frontendInput) === 3) {
                $this->attributeMap[$productId][$attribute->getId()]['frontend_input'] =
                    $frontendInput['swatch_input_type'];
            } else {
                $this->attributeMap[$productId][$attribute->getId()]['frontend_input'] =
                    $attribute->getProductAttribute()->getFrontendInput();
            }
