...
const getResolveLoader = require('@magento/pwa-buildpack/lib/WebpackTools/configureWebpack/getResolveLoader');

const RootComponentsPlugin = require('@magento/pwa-buildpack/lib/WebpackTools/plugins/RootComponentsPlugin');
const ServiceWorkerPlugin = require('@magento/pwa-buildpack/lib/WebpackTools/plugins/ServiceWorkerPlugin');
const UpwardIncludePlugin = require('@magento/pwa-buildpack/lib/WebpackTools/plugins/UpwardIncludePlugin');
...
async function getClientConfig(opts) {
    ...
    if (vendor.length > 0) {
        const optimizedVendor = [
            ...
            'redux-actions',
            'redux-thunk',
            // Move some packages to vendors instead of client chunk
            '@formatjs',
            '@react-aria',
            'react-router',
            'react-helmet',
        ...
        vendorTest += `(${optimizedVendor.join('|')})[\\\/]`;
    }
    ...
    if (mode === 'development') {
        ...
        if (isDevServer()) {
            ...
            const PWADevServer = require('@magento/pwa-buildpack/lib/WebpackTools/PWADevServer');
