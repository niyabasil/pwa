...
// };
const deserializeShippingMethod = (serializedValue) => {
    const index = serializedValue.indexOf('_');
    if (index === -1) return [serializedValue]; 
    return [
        serializedValue.substring(0, index),
        serializedValue.substring(index + 1)
    ];
};
...
export const useShippingMethod = (props = {}) => {
    ...
    const allowedfreeshippingData = useMemo(
         () =>
            data &&
         data.cart.items &&
    ...
    const shippingMethods = useMemo(() => {
        ...
        const filteredMethods = rawShippingMethods.filter(m => m.available);
        const shippingMethodsByPrice = [...filteredMethods].sort(byPrice);
