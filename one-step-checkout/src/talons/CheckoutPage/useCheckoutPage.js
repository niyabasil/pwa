...
import EWAY_URL_OPERATIONS from '@eway/eway/src/talons/ewayurl.gql.js';
...
export const useCheckoutPage = (props = {}) => {
    ...
    //     zipLoading,
    //     zipError
    // } = useQuery(getZippaymentUrl,{
    //     variables: {
    //         cartId
    //     }
    // });
    const [getzipdata, { data: zipDetailsData }] = useLazyQuery(
        getZippaymentUrl,
        {
    ...
    const [fetchEwayUrl, { loading: ewayLoading, error: ewayError }] =
    useLazyQuery(EWAY_URL_OPERATIONS.getEwayUrl, { fetchPolicy: 'no-cache' });
    ...
    const runPlaceOrderWorkFlow = useCallback(async () => {
        try {
            //alert(JSON.stringify("heiii"));
            if (cartId) {
               // alert(JSON.stringify("heloo"));
                const variables = { cartId };
                    // Commented by Niya//
                    //alert(JSON.stringify("entered"));
                // if (data && data.cart && data.cart.selected_payment_method.title === 'Credit Card - Eway') {
                //     const zipUrlResponse = await getzipdata({ variables: { cart_id: cartId } });
                //     const paymentRedirectUrl = zipUrlResponse?.data?.zippaymentPaymentUrl?.payment_link;
                //     //alert(JSON.stringify(paymentRedirectUrl));
                ...
                if (data && data.cart && data.cart.selected_payment_method.code === 'zippayment') {
                    const zipUrlResponse = await getzipdata({ variables: { cart_id: cartId } });
                    const paymentRedirectUrl = zipUrlResponse?.data?.zippaymentPaymentUrl?.payment_link;
                    //alert(JSON.stringify(paymentRedirectUrl));
                    if (paymentRedirectUrl) {
                ...
                }else if (data && data.cart && data.cart.selected_payment_method.code === 'ewayrapid') {
                    
                    const ewayUrlResponse = await fetchEwayUrl({ variables: { cart_id: cartId } });
                    const ewayPaymentUrl = ewayUrlResponse?.data?.ewayPaymentRequestData?.eway_payment_url;
                    if (ewayPaymentUrl) {
                        setIsPlacingOrder(true);
                        window.location.href = ewayPaymentUrl; 
                        return; 
                    }
                
                }
                else{
                    const result = await placeOrder({ variables });
                }
                //end Eway//
                ...
                if (result) {
                    ...
                    const orderMultisafepayUrlData =
                        (orderData && orderData.placeOrder.order.zippayment_payment_url) || null;
    ...
    return {
        ...
        availablePaymentMethods: checkoutData?.cart ?.available_payment_methods
