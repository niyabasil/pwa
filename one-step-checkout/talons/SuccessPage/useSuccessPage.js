...
export const useSuccessPage = props => {
    ...
    const OrderData = usePriceSummary(data);
   
    const { flatData } = OrderData;
    ...
    useEffect(() => {
        ...
        const items = props.data.cart.items.map(item => ({
            item_id: item.product.id,
            item_name: item.product.name,
            price: item.prices.price.value,
            quantity: item.quantity
        }));
        const grandTotal = total && total.value ? total.value : null;
        window.gtag('event', 'purchase', {
            transaction_id: orderNumber,
            affiliation: 'Online Store',
            value: grandTotal,
            currency: 'AUD',
            //tax: props.data.cart.prices.applied_taxes.reduce((acc, tax) => acc + tax.amount.value, 0),
            //shipping: OrderData.shipping.selected_shipping_method.amount.value,
            items: items
        });
